...
val_rel <- apply(Rel_and_SC_excluded[, 77:87], 1, sum)  
table(val_rel)

#range: 11 - 55

#add val science as a column in a dataframe
Rel_and_SC_excluded <- add_column(Rel_and_SC_excluded, val_rel = val_rel, .after = "SVC11")
table(Rel_and_SC_excluded$val_rel)


#country 1 2 with val science
C1_rel <- subset(Rel_and_SC_excluded, Country == "1")
C2_rel <- subset(Rel_and_SC_excluded, Country == "2")


#mean and sd
mean(Rel_and_SC_excluded$val_rel, na.rm = TRUE)
#39.81234

sd(Rel_and_SC_excluded$val_rel, na.rm = TRUE)
#10.79224
...
#GGPPLOT OF MEANS 
ggplot(Rel_and_SC_excluded, aes(x = factor(Country), y = val_rel)) +
  geom_violin() +
  stat_summary(fun = mean, geom = "point", shape = 18, size = 4, color = "red", 
               position = position_dodge(width = 0.75))+
